...
//!
//! Not the query, not the cursor, not a log line, and never a value. A
//! session file is a layout, and a layout is the only thing worth putting
//! back.
//!
//! Anything this build does not recognise — a column key from a newer
//! version, a tab the file no longer names — is dropped rather than refused:
//! a session is a convenience, and an unreadable one costs a default layout,
//! not a start.
...
/// Bumped when a stored layout would read wrong.
const VERSION: u32 = 1;
...
pub struct Session {
    ...
    #[serde(default)]
    pub version: u32,
    /// The open tab's label, `qa/dev`. One `config.toml` no longer names
...
mod tests {
    ...
    fn session() -> Session {
        let mut session = Session {
            tab: Some("qa/dev".into()),
            ..Session::default()
        };
        let tab = session.tab("qa/dev");
    ...
    fn a_session_round_trips_through_the_file() {
        ...
        assert_eq!(read.tab.as_deref(), Some("qa/dev"));
        let tab = &read.tabs["qa/dev"];
        assert_eq!(
            tab.sort
                .as_ref()
    ...
    fn a_file_this_build_cannot_read_is_the_default_layout() {
        ...
        std::fs::write(&path, r#"{"version":99,"tab":"qa/dev"}"#).unwrap();
        assert!(
    ...
    fn a_column_key_this_build_does_not_know_is_carried_but_never_applied() {
        ...
        std::fs::write(
            &path,
            r#"{"version":1,"tab":"qa/dev","tabs":{"qa/dev":{"columns":[{"key":"from_the_future","width":9}]}}}"#,
        )
        .unwrap();
        let read = Session::load(&path);
        assert_eq!(read.tabs["qa/dev"].columns[0].key, "from_the_future");
    }
    ...
    fn nothing_that_was_typed_or_read_reaches_the_file() {
        ...
        // the check from the outside that keeps it that way.
        for absent in ["query", "cursor", "orders-api", "\"value\"", "log"] {
