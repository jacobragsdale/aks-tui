...
/// `$XDG_CONFIG_HOME/aks-tui`, else `~/.config/aks-tui` — on macOS too, because
...
/// `$XDG_DATA_HOME/aks-tui`, else `~/.local/share/aks-tui`, and
/// `~/Library/Application Support/aks-tui` on macOS.
...
/// The config file this run reads: `--config` first, then `AKS_TUI_CONFIG`,
...
pub fn config_file_with(
    ...
    if let Some(path) = env("AKS_TUI_CONFIG").filter(|value| !value.is_empty()) {
        return PathBuf::from(path);
    }
    config_dir_with(env).join("config.toml")
}

/// The cache file: `--cache` first, then `AKS_TUI_CACHE`, then the data
...
pub fn cache_file_with(
    ...
    if let Some(path) = env("AKS_TUI_CACHE").filter(|value| !value.is_empty()) {
        return PathBuf::from(path);
    }
...
mod tests {
    ...
    fn xdg_wins_over_home_and_a_relative_value_is_ignored() {
        ...
        assert_eq!(config_dir_with(xdg), PathBuf::from("/xdg/aks-tui"));

        let home_only = env_of(&[("HOME", "/home/j")]);
        assert_eq!(
            config_dir_with(home_only),
            PathBuf::from("/home/j/.config/aks-tui")
        );
    ...
    fn the_flag_beats_the_variable_which_beats_the_directory() {
        let env = env_of(&[
            ("HOME", "/home/j"),
            ("AKS_TUI_CONFIG", "/etc/az.toml"),
            ("AKS_TUI_CACHE", "/var/az.json"),
        ]);
        assert_eq!(
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/j/.config/aks-tui/config.toml")
        );
        assert!(session_file_with(bare).ends_with("aks-tui/session.json"));
    }
}
