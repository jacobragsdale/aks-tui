...
pub struct Theme {
    ...
    /// `error`: a secret that is about to lapse is a fact about the clock,
    /// not a failure.
    pub warning: Color,
    pub error: Color,
    pub scrollbar: Color,
...
impl Theme {
    ...
    /// under `NO_COLOR`, otherwise whichever preset `AKS_TUI_THEME` names, and
    /// the terminal's own colours when it names none.
    ///
    ...
    pub fn from_env() -> Self {
        ...
        match std::env::var("AKS_TUI_THEME")
            .ok()
            .as_deref()
            .map(str::trim)
        {
...
impl ThemeChoice {
    ...
    /// `--theme` or `AKS_TUI_THEME` settled on, then the file's `preset`,
...
/// `--theme` before `AKS_TUI_THEME`; a name neither of the four is a startup
/// error, so a typo is said out loud rather than silently ignored.
...
"##,
    ...
    #[test]
    fn every_coloured_preset_keeps_a_warning_apart_from_a_failure() {
