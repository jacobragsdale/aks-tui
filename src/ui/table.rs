...
// line of tag badges under the title. Nothing in aks-tui has a second line to
...
mod tests {
    ...
    fn pods() -> Vec<Vec<Cell>> {
        vec![
            row(&["orders-api-7d9f5b-abc12", "1/1", "● Running", "0", "2d"]),
            row(&["orders-api-7d9f5b-k9x2p", "1/1", "● Running", "0", "2d"]),
            row(&[
                "orders-worker-5c4d-q8zt",
                "0/1",
                "✗ CrashLoopBackOff",
                "7",
                "1h",
        ...
        terminal
            .draw(|frame| {
                let mut spec = TableSpec {
                    title: " Pods ".to_owned(),
                    status: format!("{}/41 \u{b7} Name \u{2191}", rows.len()),
                    focused: true,
                    columns: &columns,
    ...
    fn the_table_says_what_it_is_on_one_border_and_what_it_is_doing_on_the_other() {
        ...
        let (buffer, hits) = draw(90, 10, &pods(), &mut cursor, |_| {});
        let drawn = screen_text(&buffer);

        assert!(drawn.contains(" Pods "), "{drawn}");
        assert!(drawn.contains("3/41 \u{b7} Name \u{2191}"), "{drawn}");
        assert!(drawn.contains("Name \u{2191}"), "the sorted header {drawn}");
        assert!(drawn.contains("Status"), "{drawn}");
        ...
        let (_, name) = hits.headers[0];
        assert_eq!(
            buffer[(name.x, hits.rows[0].1.y)].symbol(),
            "o",
            "the Name header sits over the names"
        );
        assert_eq!(
            cursor.scroll.viewport, 6,
            "ten rows less two borders, the header and its rule"
        );
        assert_eq!(cursor.scroll.content, 41);
    }
    ...
    fn the_row_under_the_cursor_is_marked_and_the_one_under_the_pointer_is_washed() {
        ...
        table_geometry(Rect::new(0, 0, 90, 10)).window(&mut cursor, 41);
        cursor.focus(1);
        let (buffer, hits) = draw(90, 10, &pods(), &mut cursor, |spec| spec.hovered = Some(2));
    ...
    fn a_long_list_shows_the_window_it_was_handed_and_the_rest_stays_off_screen() {
        let all: Vec<Vec<Cell>> = (0..40)
            .map(|index| row(&[&format!("pod-{index:02}"), "1/1", "Running", "0", "1d"]))
            .collect();
        let mut cursor = ListCursor::default();
        ...
        assert!(drawn.contains("pod-30"), "{drawn}");
        assert!(!drawn.contains("pod-00"), "{drawn}");
        assert_eq!(
    ...
    fn a_table_with_no_room_for_a_row_still_draws_its_frame() {
        let mut cursor = ListCursor::default();
        let (buffer, hits) = draw(90, 3, &pods(), &mut cursor, |_| {});
        assert!(screen_text(&buffer).contains(" Pods "));
    ...
    fn a_cell_a_column_was_dropped_out_from_under_is_not_drawn() {
        // A screen paints one cell per column it was told about; the pods
        // table in 40 cells has room for the name and the status only.
        let layout = TableLayout::new(POD_COLUMNS);
        let columns = layout.visible_columns(TableLayout::available_width(38));
        assert_eq!(columns.len(), 2, "{columns:?}");

        let mut cursor = ListCursor::default();
        let rows = vec![row(&["orders-api-7d9f5b-abc12", "● Running"])];
