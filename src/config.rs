//! `config.toml`: which clusters and namespaces to show, how often to read
//! them, and how the TUI should look. It lives in `$XDG_CONFIG_HOME/aks-tui/`
//! (`~/.config` by default).
//!
//! ```toml
//! [[clusters]]
...
//! ```
//!
//! Every tab is one namespace of one cluster, in the order the file names
//! them. A cluster that lists no namespaces is one tab over all of them.
//! A flag or an `AKS_TUI_*` variable still wins over the file. Keys this build
//! does not know are ignored. The `[theme.custom]` table is byte-for-byte
//! ticket-tui's and az-tui's, so the `theme` tool writes one file all three
//! read.
...
pub struct Config {
    ...
    /// Seconds between reads of the open tab. `Some(0)` turns the timer off
...
/// One tab: one namespace of one cluster, or every namespace of one.
...
/// `namespaces = "prod"` and `namespaces = ["prod"]` both read: one is the
...
"#;
    ...
    fn a_missing_file_is_the_default_configuration() {
        let config = load(Path::new("/nonexistent/aks-tui/config.toml")).unwrap();
